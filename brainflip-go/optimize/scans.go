...
func Optimize_scans(instructions *[]lp.Instruction) {
	...
	for i := len(scans) - 1; i >= 0; i-- {
		...
		ri.Add_instr("movdqu  xmm1, [zeroes] ; load 0s")
		ri.Add_instr("pcmpeqb xmm0, xmm1 ;  zero_count = CMEQ.16B input, 0s; all 0s marked with -1 else 0")

		ri.Add_instr("; if ecx 16, loop")
		ri.Add_instr("; else adjust rdi (p[0]) by ecx")

		ri.Add_instr("movdqu  xmm1, [ones]")
		ri.Add_instr("pxor    xmm0, xmm1 ; not_zero_count = xor 0xFF.., zero_count")
		ri.Add_instr("movdqu  xmm1, [indices_%d]", abs(scan.Moves))
		ri.Add_instr("por     xmm0, xmm1 ; masked = ORN.16B indices, zeroes")
