...
func Optimize_scans(instructions *[]lp.Instruction) {
	...
	for i := len(scans) - 1; i >= 0; i-- {
		...
		ri.Add_instr("vmovdqu  ymm1, [ones]")
		ri.Add_instr("vmovdqu  ymm2, [zeroes] ; load 0s")
		ri.Add_instr("vmovdqu  ymm3, [indices_%d]", abs(scan.Moves))

		ri.Add_label("left_vector_%d", l)
		ri.Add_instr("vmovdqu  ymm0, [rdi] ; load data from tape")
		ri.Add_instr("vpcmpeqb ymm0, ymm2 ;  zero_count = CMEQ.16B input, 0s; all 0s marked with -1 else 0")
		// ri.Add_instr("pxor    ymm0, ymm1 ; not_zero_count = xor 0xFF.., zero_count")
		// ri.Add_instr("por     ymm0, ymm3 ; masked = ORN.16B indices, zeroes")
		ri.Add_instr("vpmovmskb eax, ymm0 ; mov msb of ymm0 into eax, now data is in bits instead of bytes")
		// ri.Add_instr("xor      eax, 0xFFFFFFFF")
		// TODO MASK WITH INDEX COUNT
